...
class WebDavService {
  ...
  WebDavService({http.Client? client, WebDavConfigService? configService})
  ...
  Future<File> download(String remotePath, {String? localName}) async {
    ...
    final response = await _client
        .get(_remoteUri(remotePath, config), headers: _authHeaders(config))
    ...
    final name =
        localName ??
        '${DateTime.now().microsecondsSinceEpoch}_${remotePath.split('/').last}';
...
<D:lockinfo xmlns:D="DAV:">
  ...
  Future<void> unlock(String remotePath, String lockToken) async {
    ...
    final response = await _send(
      ...
      remotePath,
      config,
      headers: {..._authHeaders(config), 'Lock-Token': lockToken},
      allowBody: false,
    );
    if (response.statusCode < 200 || response.statusCode >= 300) {
  ...
  Uri _remoteUri(String remotePath, WebDavConnectionConfig config) {
    final normalizedBase = config.baseUrl.endsWith('/')
        ? config.baseUrl
        : '${config.baseUrl}/';
  ...
  Map<String, String> _authHeaders(WebDavConnectionConfig config) {
  ...
  Future<http.Response> _send(
    ...
    final request = http.Request(method, _remoteUri(remotePath, config));
  ...
  Future<WebDavConnectionConfig> _requireConfig() async {
    final config = await _configService.load();
    if (!config.isConfigured) {
      throw const WebDavException('请先在登录页左下角配置 WebDAV');
    }
    return config;
  }
