...
class _AiCharacterGenerateScreenState extends State<AiCharacterGenerateScreen> {
  ...
  String get _systemPrompt =>
      '你是专业的 AI 小说角色扮演角色卡创作助手。你正在为一个 AI 小说扮演功能创建可直接用于对话的角色设定。你必须只输出一个合法 JSON 对象，不要输出 Markdown、解释、代码块或额外文字。';

  String _buildPrompt(String scene, int minChars) {
    return '''
根据用户给出的场景描述，创建一个可用于 AI 小说角色扮演聊天的角色卡。角色卡需要服务于长期剧情互动，而不是普通问答助手。
...
2. name 是角色名称，应贴合场景、时代背景和人物身份。
3. personality 必须不少于 $minChars 个中文字符，用结构化段落编写，避免整段堆成一大坨文字。
4. personality 建议按需包含以下内容，不要求每个角色都全部具备，但要优先保证角色可扮演、可互动、可推动剧情：
   - 【角色身份】：身份、职业、阵营、社会地位或在故事中的定位。
   - 【性格特征】：核心性格、情绪表达方式、行为习惯、价值观、弱点或反差。
   - 【外貌气质】：容貌、衣着、声音、神态、气场等能帮助用户形成画面感的描写。
   - 【背景经历】：过往经历、秘密、创伤、执念、目标或与当前剧情有关的原因。
   - 【关系设定】：角色与用户的关系、称呼、熟悉程度、矛盾、羁绊或暧昧张力。
   - 【剧情开局】：当前场景、冲突、角色正在做什么，以及后续可展开的剧情钩子。
   - 【说话风格】：语气、措辞习惯、口头禅、对用户说话时的态度。
   - 【互动边界】：角色会主动推进剧情，但不要代替用户决定动作、台词或心理。
   - 【世界观要点】：只在场景需要时补充时代、组织、规则、力量体系等背景。
5. personality 的格式应尽量清晰，例如使用【角色身份】【性格特征】这类小标题和分段；可以有适度细节，但不要写成列表模板的空话。
6. greeting 是角色第一句开场白，应能直接开启剧情，包含场景动作、氛围描写和一句对用户说的话。
7. myNickname 是用户在故事中的称呼，默认可以是“你”或贴合场景的身份称呼。
8. aiNickname 是角色对自己的称呼或角色名。
9. 内容必须中文。
10. 不要把字段写成数组，不要省略字段，不要输出 JSON 以外的任何内容。
''';
  }
