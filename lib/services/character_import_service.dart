...
class CharacterImportService {
  ...
  /// 从 PNG 文件导入
  static Future<CharacterImportResult?> _importFromPng(
    PlatformFile file,
  ) async {
    ...
    // 保存图片到应用目录作为头像
    final avatarPath = await saveAvatarImage(bytes, cardData.name);
  ...
  /// 从 JSON 文件导入
  static Future<CharacterImportResult?> _importFromJson(
    PlatformFile file,
  ) async {
    final bytes = file.bytes;
    if (bytes == null) return null;
  ...
  /// 选择一张本地图片并保存为角色头像，用户取消返回 null。
  static Future<String?> pickAvatarImage({String? name}) async {
    final result = await FilePicker.platform.pickFiles(
      type: FileType.custom,
    ...
    final ext = file.extension?.toLowerCase();
    final imageExt = switch (ext) {
      'jpg' || 'jpeg' => 'jpg',
      'webp' => 'webp',
      _ => 'png',
    };
    final trimmedName = name?.trim() ?? '';
    final avatarName = trimmedName.isEmpty ? 'avatar' : trimmedName;

    return saveAvatarImage(bytes, avatarName, extension: imageExt);
  }
  ...
  static Future<String> saveAvatarImage(
    ...
    final filePath =
        '${avatarDir.path}/imported_${safeName}_$timestamp.$extension';
